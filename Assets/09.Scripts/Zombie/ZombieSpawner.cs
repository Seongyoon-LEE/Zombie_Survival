...
public class ZombieSpawner : MonoBehaviourPun, IPunObservable
{
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        else
        {
            ...
            // byte, bool, int, float, string -> byte[] 직렬화
            // byte -> bool, int, float 역직렬화
            // PUN2 에서 Color 타입은 RPC 메서드의 입력으로 첨부 할 수 없다.
            // 기존의 RPC에서 지원하지 않은 타입을 직접 지원하도록 하는 것이
            // PhotonPeer.RegisterType(타입, 번호, 직렬화 메서드, 역직렬화 메서드) 메서드 이다
        }
    }

    private void Awake()
    {
        //좀비 랜더러 컬러를 직렬화 역직렬화 하는 메서드
